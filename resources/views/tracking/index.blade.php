...
@section('content')
    ...
    <section class="p-10">
        ...
        <div class="w-fit mx-auto flex items-center my-10 space-x-5">
            @if(isset($tracking))
            @foreach ($tracking_states as $key=>$status)
            @if($status)
                <div class="text-center text-xs">
                    <div class="bg-{{$tracking->$key == null ? 'gray-400':'blue-800';}} mx-auto rounded-full w-fit px-2 h-10 relative
                    ...
                    <h3>{{$tracking->$key == null ? '-':$tracking->$key;}}</h3>
                </div>
            @endif
            @endforeach
            @endif
...
@push('child-scripts')
        <script type="text/javascript">
            ...
            let position = {!! json_encode($last_status) !!};
